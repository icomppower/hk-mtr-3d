...
import { buildUnit, buildArrow, buildRain, buildTrains, updateUnits, updateArrows, updateFlags,
         updateEffects, updateTrains, applyWeather, stepRain, unitObjs, arrowObjs } from "./entities.js";
...
function frame(dt){
  ...
  updateTrains(); updateFlags(); updateEffects(Clock.day,dt); stepRain(dt,w); updateLabels();
  renderScene();
}
...
(async function init(){
  try{
    ...
    await loadTiles();
    buildTerrain(); buildLabels(); buildLine(); buildTrains(); buildRain();
